...
void Fuzzy()
{
	...
	// 1ª regra - If temperatura is poor or distancia is rancid, tip is cheap
	...
	// 2.1ª regra - If temperatura is good, tip is average
	...
	// 3.1ª regra - If temperatura is excellent or distancia is delicious, tip is generous
...
void __fastcall TFmFuzzy::FormCreate(TObject *Sender)
{
	...
	x = 23; // Inicializa x em 21 para corresponder ao início do seu eixo X
	for (int a = 0; a <= 10; a++) { // Itera do valor inicial até 10 passos à frente
		...
		Chart1->Series[0]->AddXY(x, trapmf(x, 22, 23, 24, 27)); // Substitua pelos parâmetros corretos
		Chart1->Series[1]->AddXY(x, trimf(x, 24, 28, 32));      // Substitua pelos parâmetros corretos
		Chart1->Series[2]->AddXY(x, trapmf(x, 29, 32, 33, 34)); // Substitua pelos parâmetros corretos
...
void __fastcall TFmFuzzy::VaComm1RxChar(TObject *Sender, int Count)
{
	...
	VaComm1->ReadBuf(buffer, Count); // Lê os dados recebidos
	...
	if (indexD > 0 && indexT > indexD) {
		...
		// Converta e use os valores de distância e temperatura como necessário
